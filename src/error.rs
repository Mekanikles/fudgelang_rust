...
pub fn new_non_utf8_sequence_error(pos: u64, len: u64) -> Error {
    Error::at_span(ErrorId::NonUtf8Sequence, source::SourceSpan { pos, len: len as usize }, "Found non-ut8 sequence".into())
}

pub fn new_unexpected_char_error(c: char, pos: u64) -> Error {
    Error::at_pos(ErrorId::UnexpectedChar, pos, format!("Found unexpected character '{}'", c))
}

pub fn new_non_ascii_char_error(c: char, pos: u64) -> Error {
    Error::at_pos(ErrorId::NonAsciiChar, pos, format!("Found non-ascii character '{}' outside of string literal", c))
}

pub fn new_unexpected_sequence_error(pos: u64, len: u64, message: String) -> Error {
    Error::at_span(ErrorId::EnexpectedSequence, source::SourceSpan { pos, len: len as usize }, message)
}