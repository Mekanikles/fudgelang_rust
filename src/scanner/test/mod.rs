...
fn expect_error_ids(errors : &Vec<error::Error>, expected_error_ids: &[error::ErrorId]) {
    ...
    assert_eq!(errors.len(), expected_error_ids.len(), "Found more errors than expected!");
}
...
fn verify_scanner_tokens<S: Scanner>(scanner: &mut S, expected_tokens: &[Token])
{
    ...
    assert_eq!(count as usize, expected_tokens.len(), "Scanned less tokens than expected!");    
}
