...
fn test_non_ascii_chars() {
    let errors = verify_exact_scan(".ö.", &[
        Token::new(TokenType::Dot, 0, 1),
        Token::new(TokenType::Dot, 3, 1) ]);
    expect_error_ids(&errors, &[
        error::ErrorId::NonAsciiChar ]);
}

#[test]
fn test_unexpected_chars() {
    let errors = verify_exact_scan(".\0.", &[
        Token::new(TokenType::Dot, 0, 1),
        Token::new(TokenType::Dot, 2, 1) ]);
    expect_error_ids(&errors, &[
        error::ErrorId::UnexpectedChar ]);
}
...
fn test_non_utf8_sequence() {
    ...
    let source = MemorySource::from_bytes(&[
        b'.', ILLEGAL_CTRL_BYTE,
        b'.', CTRL_2_BYTE, ILLEGAL_SEQ_BYTE,
        b'.', CTRL_3_BYTE, SEQ_BYTE, ILLEGAL_SEQ_BYTE,
        b'.', CTRL_4_BYTE, SEQ_BYTE, SEQ_BYTE, ILLEGAL_SEQ_BYTE,
        b'.']);
    let mut scanner = ScannerImpl::new(&source);

    verify_scanner_tokens(&mut scanner, &[
        Token::new(TokenType::Dot, 0, 1),
        Token::new(TokenType::Dot, 2, 1),
        Token::new(TokenType::Dot, 5, 1),
        Token::new(TokenType::Dot, 9, 1),
        Token::new(TokenType::Dot, 14, 1) ]);
    expect_error_ids(&scanner.errors, &[
